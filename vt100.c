...
void _vt100_clearLines(struct vt100 *t, uint16_t start_line, uint16_t end_line){
	for(int c = start_line; c <= end_line; c++){
		for(int i = 0 ; i < 32; i++) vfb[c*32+i] = 0;
	}
...
void _vt100_putc(struct vt100 *t, uint8_t ch){
	...
	uint16_t x = VT100_CURSOR_X(t)/8;
	uint16_t y = VT100_CURSOR_Y(t)/8;
...
void ili9340_fillRect(int x, int y, int w, int h, int nil)
{
	x /= 8;
	y /= 8;
	w /= 8;
	h /= 8;
	for(int i = x; i < w; i++)
		for(int j = y; j < h; j++) vfb[j * 32 + i] = 0;
}
